...
ggplot() +
  ...
  scale_fill_viridis_c(option = "plasma", na.value = "grey90", name = "Number of Students") +
  geom_sf(data = geocoded_sf, color = "black", size = 1, alpha = 0.5) +
  theme_minimal() +
  annotation_scale(location = "br", width_hint = 0.25, text_cex = 0.8) +   # Scale bar
  annotation_north_arrow(location = "tr", which_north = "true",
                         style = north_arrow_fancy_orienteering) +        # Compass
...
set.seed(8)
n_sim <- 1000
n_random <- nrow(newspapers_proj)

median_dists <- numeric(n_sim)

for (i in 1:n_sim) {
  random_points <- st_sample(dk_proj, size = n_random, type = "random") %>% st_as_sf()
  dist_random <- st_distance(random_points, geocoded_proj)
...
```{r}
results <- mean(median_dists <= real_median)
results
```
